...
const columns = ['slots_won', 'blackjack_won', 'roulette_won', 'crash_won', 'mines_won', 'dice_won', 'coinflip_won', 'corrida_won', 'last_daily', 'xp', 'level', 'daily_streak'];
...
export const recordBet = (id: string, betAmount: number, winAmount: number, gameType?: 'slots' | 'blackjack' | 'roulette' | 'coinflip' | 'crash' | 'mines' | 'dice' | 'corrida') => {
  getUser(id);
...
export const getLeaderboard = (category: 'balance' | 'total_won' | 'slots_won' | 'blackjack_won' | 'roulette_won' | 'coinflip_won' | 'corrida_won' | 'level', limit: number = 10) => {
...
export const updateCooldown = (id: string, type: 'daily') => {
